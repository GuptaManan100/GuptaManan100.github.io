...
description: "In a single database, prepared statements parse once. Behind many gateways that win quietly evaporates. How Multigres consolidates prepared statements so Postgres parses each one exactly once."
publishDate: "2026-05-20T12:00"
liveDate: "2026-06-20"
tags: ["postgres", "multigres", "connection-pooling", "prepared-statements", "distributed-systems"]
seriesId: "Multigres Connection Pooling"
orderInSeries: 4
---

**Originally published on May 20, 2026 at [multigres.com](https://multigres.com/blog/one-parse-per-query-no-matter-how-many-gateways)**

---

# One parse per query, no matter how many gateways

_Part 4 of a series. Start with [Two jobs, two processes](/bytes/two-jobs-two-processes) for the architecture and motivation._
...
<img src="/files/connection-pooling/n-gateways-problem-light.png" alt="Before vs. after the N-gateways problem. Before: three clients on three gateways each forward their own PREPARE, producing three differently-named entries in pg_prepared_statements. After: multipooler interns the query and forwards a single PREPARE ppstmt0 plus three EXECUTE ppstmt0 calls; the backend holds one entry per backend connection, not per (gateway, client-name) pair." class="dark:hidden" />
<img src="/files/connection-pooling/n-gateways-problem-dark.png" alt="Before vs. after the N-gateways problem. Before: three clients on three gateways each forward their own PREPARE, producing three differently-named entries in pg_prepared_statements. After: multipooler interns the query and forwards a single PREPARE ppstmt0 plus three EXECUTE ppstmt0 calls; the backend holds one entry per backend connection, not per (gateway, client-name) pair." class="hidden dark:block" />
...
<img src="/files/connection-pooling/dedup-flow-light.png" alt="Sequence diagram across Client A, Gateway A, Multipooler, Postgres, then Client B / Gateway B. Cold-cache path: Client A prepares and executes; gateway forwards EXECUTE with query text + param types; multipooler misses its canonical-name map, sends PREPARE ppstmt0 then EXECUTE to Postgres. Warm-cache path: Client B prepares the same query text on a different gateway; multipooler hits its map and sends only EXECUTE ppstmt0 — no second PREPARE." class="dark:hidden" />
<img src="/files/connection-pooling/dedup-flow-dark.png" alt="Sequence diagram across Client A, Gateway A, Multipooler, Postgres, then Client B / Gateway B. Cold-cache path: Client A prepares and executes; gateway forwards EXECUTE with query text + param types; multipooler misses its canonical-name map, sends PREPARE ppstmt0 then EXECUTE to Postgres. Warm-cache path: Client B prepares the same query text on a different gateway; multipooler hits its map and sends only EXECUTE ppstmt0 — no second PREPARE." class="hidden dark:block" />
...
1. [Two jobs, two processes: why Multigres has its own connection pooler](/bytes/two-jobs-two-processes) - the architecture and motivation.
2. [Per-user pools that share fairly](/bytes/per-user-pools-that-share-fairly) - security, fair sharing, and reuse across session settings.
3. [Pooling without choosing a mode](/bytes/pooling-without-choosing-a-mode) - transaction-mode speed and session-mode safety, automatic.
