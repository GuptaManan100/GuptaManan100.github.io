...
description: "Why Multigres gives every user a dedicated connection pool (mostly a security story), how it shares a fixed budget fairly with max-min fairness, and the bucketing trick that keeps lookup fast."
publishDate: "2026-05-20T10:00"
liveDate: "2026-06-20"
tags: ["postgres", "multigres", "connection-pooling", "security", "fairness"]
seriesId: "Multigres Connection Pooling"
orderInSeries: 2
---

**Originally published on May 20, 2026 at [multigres.com](https://multigres.com/blog/per-user-pools-that-share-fairly)**

---

# Per-user pools that share fairly

_Part 2 of a series. Start with [Two jobs, two processes](/bytes/two-jobs-two-processes) for the architecture and motivation._
...
<img src="/files/connection-pooling/per-user-pool-share-round-1-light.png" alt="Round 1: allocate 2 connections to alice, bob, and charlie each. 6 of 12 used." class="dark:hidden" />
<img src="/files/connection-pooling/per-user-pool-share-round-1-dark.png" alt="Round 1: allocate 2 connections to alice, bob, and charlie each. 6 of 12 used." class="hidden dark:block" />
...
<img src="/files/connection-pooling/per-user-pool-share-round-2-light.png" alt="Round 2: bob and charlie rise to 5 each, alice stays at 2. All 12 connections used." class="dark:hidden" />
<img src="/files/connection-pooling/per-user-pool-share-round-2-dark.png" alt="Round 2: bob and charlie rise to 5 each, alice stays at 2. All 12 connections used." class="hidden dark:block" />

**Round 3.** Only charlie still wants more, but the pool is exhausted. Algorithm halts. Final: alice=2, bob=5, charlie=5.

<img src="/files/connection-pooling/per-user-pool-share-round-3-light.png" alt="Round 3: pool is exhausted; charlie's remaining demand goes unsatisfied. Final allocation alice=2, bob=5, charlie=5." class="dark:hidden" />
<img src="/files/connection-pooling/per-user-pool-share-round-3-dark.png" alt="Round 3: pool is exhausted; charlie's remaining demand goes unsatisfied. Final allocation alice=2, bob=5, charlie=5." class="hidden dark:block" />
...
<img src="/files/connection-pooling/side-by-side-comparison-light.png" alt="Side-by-side: naive equal split (alice=4, bob=4, charlie=4 — 2 wasted) versus max-min (alice=2, bob=5, charlie=5 — fully utilized)." class="dark:hidden" />
<img src="/files/connection-pooling/side-by-side-comparison-dark.png" alt="Side-by-side: naive equal split (alice=4, bob=4, charlie=4 — 2 wasted) versus max-min (alice=2, bob=5, charlie=5 — fully utilized)." class="hidden dark:block" />
...
<img src="/files/connection-pooling/connection-stacks-light.png" alt="Eight stacks numbered 0–7. alice/analytics (bucket 17) and bob/sales (bucket 9) both route to stack 1 via bucket & 7; alice/reporting (bucket 22) routes to stack 6. Stack 1 holds connections for two different settings." class="dark:hidden" />
<img src="/files/connection-pooling/connection-stacks-dark.png" alt="Eight stacks numbered 0–7. alice/analytics (bucket 17) and bob/sales (bucket 9) both route to stack 1 via bucket & 7; alice/reporting (bucket 22) routes to stack 6. Stack 1 holds connections for two different settings." class="hidden dark:block" />
...
[Pooling without choosing a mode](/bytes/pooling-without-choosing-a-mode) - how Multigres gets transaction-mode speed and session-mode safety at the same time, with no upfront mode choice.
